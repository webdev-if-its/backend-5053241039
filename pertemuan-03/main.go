...
func TambahTugas(toko *TokoTugas, judul string) (Task, error) {
	...
	tugas := Task{toko.NextID, judul, false}
...
func HapusTugasTercatat(toko *TokoTugas, id int) error {
	...
	defer func() {
		if err == nil {
			catatan := fmt.Sprintf("hapus id=%d: berhasil", id)
			toko.Log = append(toko.Log, catatan)
		} else {
			catatan := fmt.Sprintf("hapus id=%d: gagal", id)
			toko.Log = append(toko.Log, catatan)
		}
	}()
...
func AmankanPanggilan(fn func() error) (err error) {
	defer func() {
		...
		if recover != nil {
			err = fmt.Errorf("operasi gagal: %v", recover)
		}
	}()
